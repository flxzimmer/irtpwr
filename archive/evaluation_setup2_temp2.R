...
# 3PL instead of 2PL, adding a guessing parameter of .1.

# Fetching some conditions from sim1, replacing the datasets with the alternative distribution ones
indices = sapply(sim1, function(x) x$n.items==10)

sim2 = lapply(sim1[indices], function(x) {
  x$datasets = list()
  for (i in 1:x$simruns) {
    x$datasets[[i]] = setup.data(hyp = x$hyp, n = x$n.pers,gmisspec=TRUE)
  }
  return(x)
})
