...
```{r}
h_basic <- list(res = res, unres = unres, maximizeL = maximizeL)
```


# Testing the new hypothesis

The new hypothesis can be set up using the h_basic list object as the type argument of the setup.hypothesis function. We may then calculate the power at arbitrary sample sizes.
...
hyp <- setup.hypothesis(type = h_basic, altpars = altpars)
